...
for draw_number in draw_numbers:                            # One by one replace the draw number on the cards with -1
    ...
    for card in reversed(cards):
        ...
        for row in card:
            ...
            for number in row:
                if number == int(draw_number):
                    print(str(card_index) + " " + str(row_index) + " " + str(column_index))
        ...
        if len(row_complete[0])+len(column_complete[0]) >= 1:  # We have a full card!
            if len(cards) == 1:
                print(cards)        
                card_total=0
                for row in cards[0]:
                ...
                quit()
            cards = numpy.delete(cards,card_index,0)           # Delete the full card 
        card_index-=1
