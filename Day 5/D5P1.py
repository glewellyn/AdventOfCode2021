...
vector_counter = len(vectors)-1                                                     # Remove vectors that are not horizontal or vertical
for vector in reversed(vectors):
    if not (vector[0]==vector[2] or vector[1]==vector[3]):
        vectors.pop(vector_counter)
    vector_counter -= 1
